...
<div class="relative row-span-2 w-full">
	<select
		bind:value
		multiple
		id={selectId}
		class="border-1 peer block w-full rounded-lg border-gray-300
	...
	<label
		for={selectId}
		class="absolute end-2 top-6 z-10 origin-[0] -translate-y-4 scale-75
        transform px-2 text-sm text-gray-500 duration-300
        peer-placeholder-shown:top-1/2 peer-placeholder-shown:translate-y-0
        peer-placeholder-shown:scale-100 peer-focus:top-4 peer-focus:-translate-y-4
        peer-focus:scale-75 peer-focus:px-2 peer-focus:text-blue-600"
	>
		{labelName}
	</label>
</div>
